/**
  ...
  Additionally, this document is directly copied from `autocomplete-powershell-winserver2022`
*/
...
async function update() {
  ...
  if (!fs.existsSync("./office-docs-powershell")) {
  ...
  const fileHandler = async (filePath, pathArray, file) => {
    ...
    CMDLET.push({
      ...
      rightLabel: frontMatter.attributes["Module Name"] ?? frontMatter.attributes.applicable
    });
  ...
  // The office-docs-powershell repo has it's documents spread around a few
  // folders, so we will call each one individually

  await enumerateFiles("./office-docs-powershell/exchange/exchange-ps/exchange/", [], fileHandler);
  await enumerateFiles("./office-docs-powershell/officewebapps/officewebapps-ps/office-webapps/", [], fileHandler);
  await enumerateFiles("./office-docs-powershell/skype/skype-ps/skype/", [], fileHandler);
  await enumerateFiles("./office-docs-powershell/spmt/spmt-ps/spmt/", [], fileHandler);
  await enumerateFiles("./office-docs-powershell/teams/teams-ps/", [], fileHandler);
  await enumerateFiles("./office-docs-powershell/whiteboard/whiteboard-ps/whiteboard/", [], fileHandler);
...
async function cloneDocRepo() {
  return new Promise((resolve, reject) => {
    try {
      ...
      childProcess.exec(
        "git clone https://github.com/MicrosoftDocs/office-docs-powershell",
        { shell: "cmd.exe" },
        (error, stdout, stderr) => {
          if (error) {
            throw error;
